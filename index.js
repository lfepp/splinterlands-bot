  ...
  try {
    ...
    if (winner.trim() == account) {
      const decWon = await getElementText(
        page,
        "section.player.winner span.dec-reward span",
        1000
      ).catch(() => {
...
async function commenceBattle(page) {
  ...
  await page.waitForTimeout(5000);
  const isBtnSkipVisible = await page
    .waitForSelector("#btnSkip", { timeout: btnSkipTimeout })
    .then(() => {
  ...
  await page
    .waitForSelector("#btnSkip", { timeout: 30000 })
